...
export class MysqlSaleRepository implements SaleRepository {
  async register(input: SaleInput): Promise<SaleSummary> {
    ...
    try {
      ...
      const [saleResult] = await connection.execute<ResultSetHeader>("INSERT INTO ventas (caja_turno_id, metodo_pago, cuenta_por_cobrar_id, subtotal, total, costo_total) VALUES (?, ?, ?, ?, ?, ?)", [input.cajaTurnoId, normalizedPayment, creditId, subtotal, subtotal, costoTotal]);
      ...
      if (normalizedPayment !== "credito") {
        await connection.execute("INSERT INTO movimientos_caja (caja_turno_id, tipo, metodo_pago, monto, concepto) VALUES (?, 'venta', ?, ?, ?)", [input.cajaTurnoId, normalizedPayment, subtotal, `Venta #${saleResult.insertId}`]);
      }
